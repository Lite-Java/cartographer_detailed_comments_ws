...
class RangeDataCollator {
 ...
 private:
  sensor::TimedPointCloudOriginData CropAndMerge();//同一个topic的激光帧丢掉时间重合的。不同的topic的激光帧按时间顺序融合
  ...
  common::Time current_start_ = common::Time::min();//Time::min()是chrono自带的函数。返回一个低于1970.01.01的数。
