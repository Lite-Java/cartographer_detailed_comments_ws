...
class OccupiedSpaceCostFunction2D {
 public:
  ...
  bool operator()(const T* const pose, T* residual) const {
    ...
    const MapLimits& limits = grid_.limits();//这三行是Ceres库原生提供的双三次插值迭代器。
    ...
    for (size_t i = 0; i < point_cloud_.size(); ++i) {
      ...
      interpolator.Evaluate(
          ...
          (limits.max().y() - world[1]) / limits.resolution() - 0.5 +
              static_cast<double>(kPadding),
          &residual[i]);//该函数调用有三个参数，前两个参数用来描述x,y轴索引， 第三个参数用于记录插值后的结果。这里的xy索引计算的比较奇怪，它通过GridArrayAdapter类中成员函数GetValue获取栅格数据，这里不再细述。 此外由于占用栅格中原本存储的就是栅格空闲的概率， 所以这里查询出来的概率就是(1−Msmooth(Tεhk))。
