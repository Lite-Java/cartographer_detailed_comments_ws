...
visualization_msgs::Marker CreateTrajectoryMarker(const int trajectory_id,
  ...
  marker.pose.orientation.w = 1.;//四元素第四个元素
...
int MapBuilderBridge::AddTrajectory(
  ...
  const int trajectory_id = map_builder_->AddTrajectoryBuilder(
      ...
      // lambda表达式 local_slam_result_callback_    this表示MapBuilderBridge里面的东西都可以访问
      [this](const int trajectory_id, 
        ...
        OnLocalSlamResult(trajectory_id, time, local_pose, range_data_in_local);//这个函数在本文件的最后实现的，保存local slam 的结果, 包含当前轨迹id, 当前时间, 当前位姿, 以及所有的雷达数据
      });
...
void MapBuilderBridge::RunFinalOptimization() {
  ...
  map_builder_->pose_graph()->RunFinalOptimization();//pose_graph()返回的是pose_graph2D或者pose_graph3D的原始指针，RunFinalOptimization()具体实现在pose_graph_2(3)d.cc
}
...
//为什么这里有个花括号，因为local_slam_data_ GUARDED_BY(mutex_);使用local_slam_data_需要上锁，而这个锁的作用域就在花括号内
      {
